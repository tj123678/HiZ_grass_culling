...
{
	public static Dictionary<string, string> ArabTransCaches = new Dictionary<string, string>();
	
	public static string Fix(string str)
	{
		if (ArabTransCaches.ContainsKey(str))
		{
			return ArabTransCaches[str];
		}
		
		string arabTrans = Fix(SwapTagsByRegex(str), false, true);

		if (!ArabTransCaches.ContainsKey(str))
		{
			ArabTransCaches.Add(str, arabTrans);
		}

		return arabTrans;
	}

	public static Regex Regex = new Regex(@"<\/?([a-zA-Z0-9_]+)(\s*=\s*[^>]*)*>", RegexOptions.Compiled);
	public static string SwapTagsByRegex(string input)
	{
		// 找到所有匹配的标签
		MatchCollection matches = Regex.Matches(input);

		// 如果找到的标签数量是偶数，我们可以进行交换
		if (matches.Count % 2 == 0)
		{
			...
			// 从第一个标签开始，到倒数第二个标签结束
			//这里处理顺序很重，所以要依次从后往前处理
			for (int i = matches.Count - 1; i >= 0; i--)
			{
				int endTagIndex = matches[i].Index;
				
				// 交换标签
				result.Remove(endTagIndex, matches[i].Length);
				// Debug.LogError($"remove:{matches[i]}  result:{result}");
				result.Insert(endTagIndex, matches[matches.Count - 1 - i].Value);
				// Debug.LogError($"Insert:{matches[matches.Count - 1 - i].Value}  result:{result}");
			}
	...
	public static string FixImp(string str, bool showTashkeel, bool useHinduNumbers)
	{
	...
	public static string Fix(string str, bool showTashkeel, bool useHinduNumbers)
	{
		...
		string arabTrans = FixImp(str, showTashkeel, useHinduNumbers);
...
class ArabicTable
{
	...
	/// </summary>
	public static ArabicTable ArabicMapper => arabicMapper ?? (arabicMapper = new ArabicTable());
...
class ArabicFixerTool
{
	...
	static string RemoveTashkeel(string str, out List<TashkeelLocation> tashkeelLocation)
	{
	...
	static char[] ReturnTashkeel(char[] letters, List<TashkeelLocation> tashkeelLocation)
	{
	...
	{
		// return RTLSupport.ConvertLineR(str);
		...
		List<char> list = new List<char>(lettersFinal.Length);
		...
		for (int i = lettersFinal.Length - 1; i >= 0; i--)
		{
...

			{
				numberList.Add(lettersFinal[i]);
			         ...
			         char.IsPunctuation(lettersFinal[i]) || (lettersFinal[i] >= 0x4e00 && lettersFinal[i] <= 0x9fbb )) // || lettersFinal[i] == '^') //)
			{
				numberList.Add(lettersFinal[i]);
			}
	...
	static bool IsIgnoredCharacter(char ch)
	{
	...
	static bool IsLeadingLetter(char[] letters, int index)
	{
	...
	static bool IsFinishingLetter(char[] letters, int index)
	{
	...
	static bool IsMiddleLetter(char[] letters, int index)
	{
